...
;;;;;;;;;;;;;;;;;;;;;;;;;
; arrive at lock token
;;;;;;;;;;;;;;;;;;;;;;;;;

(defrule read-token
  (declare (salience 10))
  (current-time ?time)
  ?s1 <- (token ?num ?time ?lock ?next-place)
=>
  (retract ?s1)
  (printout t ?time"  "?num"  arrives  "?lock"  "?time"  "?next-place crlf)
)
;;;;;;;;;;;;;;;;;;;;;;;;;
...
=>
  ...
  (assert (token ?num ?sum ?lock ?next-place))
...
=>
  ...
  (printout t ?time"  "?num"  enters  "?name"  "(+ ?time ?time-needed)"  "?next-place crlf)
...
=>
  (printout t ?time"  "?num"  reverses  "?lock"  "?new-time"  "?channel "  ;the reverse time is ship " ?num2" time, because it has to wait for this ship to pass"crlf)
...
=>
  (printout t ?time"  "?num"  (reversed)waiting  "?location"  "?new-time"  "?location crlf)
  (modify ?s1 (arrival-time ?new-time))
)
...
(defrule situation-ship-channel-to-goal
  ...
  (not (ship (number ?num2&:(< ?num2 ?num)) (location ?next-to-waterway) (destination ?destination)))
=>
  (printout t ?time"  "?num"  arrives  "?destination"  "?time"  " crlf)
